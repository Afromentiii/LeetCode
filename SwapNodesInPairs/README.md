...
- Złożoność czasowa to optymalne O(n), co jest wielkością optymalną i wymaganą, ponieważ musimy w najgorszym scenariuszu przejść przez każdy węzeł raz.
...
- W pliku `Solution.cs` oddzielono całkowicie sekcję parsowania tablic ze stringów od sekcji samego działania testowanego rozwiązania, zapewniając poprawność i dokładność wykonywanych pomiarów wydajności czasowej.

## Wady
- Alternatywnym sposobem rozwiązania tego zadania, często uchodzącym za dużo "czystsze" i bardziej zwięzłe w zapisie (tzw. clean code), jest zastosowanie rekurencji. Niestety, w warunkach produkcyjnych głęboka pętla rekurencyjna zaalokowałaby na stosie wywołań środowiska `CLR` dodatkową pamięć czasową rzędu O(n), co przy bardzo dużych listach groziłoby awarią `StackOverflowException`. Właśnie z tego powodu wybrana, w 100% iteracyjna implementacja z dummy node – pomimo że dłuższa w lekturze – cechuje się o wiele większą solidnością.
